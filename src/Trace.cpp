...
Trace::Trace(glm::vec2 const &initialPosition, std::shared_ptr<const GLuint> pProgram, std::shared_ptr<const GLuint> pBuffer)
    : position_{initialPosition}
    , color_{1.0, 0.0, 1.0}
    , pProgram_{pProgram}
    , pBuffer_{pBuffer}
    , creationTime_{std::chrono::steady_clock::now()}
    , deathTime_{creationTime_ + std::chrono::seconds{5}}
    , id_{++nextId_}
{
...
void Trace::step(std::chrono::milliseconds const &ms)
{
    ...
    position_ += uniformInBox(topLeft, bottomRight); // TODO: change to +=
...
std::pair<std::shared_ptr<Trace>, std::shared_ptr<Trace>> Trace::split() const
{
    return std::make_pair(
        std::make_shared<Trace>(position_, pProgram_, pBuffer_),
        std::make_shared<Trace>(position_, pProgram_, pBuffer_)
    );
}
